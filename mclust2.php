...
$g_count=0;
$t_count=0;
$getmid=mysql_query("Select b.mid,b.pid,l.latitude,l.longitude,p.capacity from belongs_to b,pool p,location l where l.mid=b.mid and b.pool_count=1 and b.pid=p.pid and p.date='$date'");//the b.pool_count=1 is because the takers who are already pooled will have the same pid as the giver and will be considered as givers themselves(spent a good hour a few days later figuring this out)
while($row=mysql_fetch_array($getmid))
{
  if($row['capacity']>0)
  {	
     $giver[$g_count]['mid']=$row['mid'];
     $mid=$row['mid'];//subscript cannot be specified in query
	...
	if($row1[$temp]!=0.0)
	{
		$giver[$g_count]['route'][$i-1]['lat']=$row1[$temp];//if the subscripting does not start at 0 js cannot calculate length.	
